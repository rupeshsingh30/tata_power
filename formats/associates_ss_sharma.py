...
def type1(data):
    ...
    for line in data:
        ...
        if len(table1) == 2:
            if 'service tax' in table1[0].lower():
                break
...
def ss_sharma_association(file_name, text, res_para, output_folder, db_conn,category, excel_id):
    
    print("--------------","ss sharma association format")
    ...
    seller_pan = ''
    seller_pan = re.search(r'(?si)\span.*branch',text).group()
    ...
    education_cess = ''
    education_cess = re.search(r'(?si)education\scess.*?secondary\sand\shigher\seducation\scess',text).group()
    education_cess = re.sub(r'(?si)secondary\sand\shigher\seducation\scess',"",education_cess)
    education_cess = re.findall(r'[0-9,.]+',education_cess)[-1]
    # print('education cess :',education_cess,'\n\n')

    he_education_cess = ''
    he_education_cess = re.search(r'(?si)secondary\sand\shigher\seducation\scess.*?out\sof\spocket',text).group()
    he_education_cess = re.findall(r'[0-9,.]+',he_education_cess)[0]
    ...
    net_value = ''
    net_value = re.search(r'(?si)total.*?remark',text).group()
