...
def type1(data):
    ...
    for line in data:
        ...
        if len(table1) == 8:
            if 'output vat' in table1[2].lower():
                break
            
            line_item = {
                'description': table1[2].strip(),
            ...
            if 'output vat' in table1[1].lower():
                break
        ...
        #     line_item = {
        #         'description': table1[1].replace('SELECTED,',"").replace('NOT_SELECTED,',"").strip(),
        #         'quantity': table1[2].replace('SELECTED,',"").replace('NOT_SELECTED,',"").strip(),
        #         'unit': table1[3].replace('SELECTED,',"").replace('NOT_SELECTED,',"").strip(),
        #         'rate': table1[4].replace('SELECTED,',"").replace('NOT_SELECTED,',"").strip(),
        #         'amount': table1[-1].replace('SELECTED,',"").replace('NOT_SELECTED,',"").strip()
        #     }
...
def retail_invoice(file_name, text, res_para, output_folder, db_conn,category, excel_id):
    
    print("--------------",'retails invoice')
    ...
    seller_pan = ''
    if re.search(r'(?si)company\'s\span.*?declaration',text):
        seller_pan = re.search(r'(?si)company\'s\span.*?declaration',text).group()
    ...
    seller_service_tax = ''
    if re.search(r'(?si)company\'s\sservice\stax\sno.*?company\'s\span',text):
        seller_service_tax = re.search(r'(?si)company\'s\sservice\stax\sno.*?company\'s\span',text).group()
        seller_service_tax = re.sub(r'(?si)company\'s\sservice\stax\sno|company\'s\span',"",seller_service_tax)
        seller_service_tax = re.search(r'(?si)[A-Z0-9]+',seller_service_tax).group()
    ...
    vat_gst = ''
    if re.search(r'(?si)output\svat.*?surcharge\son',text):
        vat_gst = re.search(r'(?si)output\svat.*?surcharge\son',text).group()
        vat_gst = re.findall(r'(?si)[0-9,.]+',vat_gst)[-1]
    # print('vat_gst :',vat_gst,'\n\n')

    vat = ''
    if re.search(r'(?si)surcharge\son\svat.*?round\sof',text):
        vat = re.search(r'(?si)surcharge\son\svat.*?round\sof',text).group()
    ...
    net_value = ''
    if re.search(r'(?si)total.*?amount',text):
        net_value = re.search(r'(?si)total.*?amount',text).group()
