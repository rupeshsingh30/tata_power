...
def type1(data):
    ...
    for line in data:
        ...
        if len(table1) == 6:
            ...
            # line_items.append(table1[0].strip())
...
def vinayak_hr_solution(file_name, text, res_para, output_folder, db_conn,category, excel_id):
    
    print("--------------",'vinayak hr solution pvt ltd')
    ...
    try:
        ...
        # buyer_name = 'South East UP Power Transmission Company Limited'
    except:
        buyer_name = ''
    ...
    service_tax = ''
    service_tax = re.search(r'(?si)service\stax.*?education\s',text).group()
    service_tax = re.findall(r'[0-9,.]+',service_tax)[-1]
    # print('servive tax :',service_tax,'\n\n')

    education_cess = ''
    education_cess = re.search(r'(?si)education\scess.*?hsec\seducation\scess',text).group()
    education_cess = re.sub(r'(?si)hsec\seducation\scess',"",education_cess)
    education_cess = re.findall(r'[0-9,.]+',education_cess)[-1]
    # print('education cess :',education_cess,'\n\n')

    he_education_cess = ''
    he_education_cess = re.search(r'(?si)hsec\seducation\scess.*?total\spayable',text).group()
    he_education_cess = re.findall(r'[0-9,.]+',he_education_cess)[-1]
    # print('he_education_cess :',he_education_cess,'\n\n')
    
    
    gross_value = ''
    gross_value = re.search(r'(?si)gross\stotal.*?service\stax',text).group()
    gross_value = re.findall(r'[0-9,.]+',gross_value)[0]
    ...
    net_value = ''
    net_value = re.search(r'(?si)net\spayable\samount.*',text).group()
    net_value = re.findall(r'(?si)[0-9,.]+',net_value)[0]
