...
def type1(data):
    ...
    for line in data:
        if not headers_skipped:
            if 'Particulars' in line or 'Amount (Rs)' in line:
...
def aws_pretty_text(output_folder,text):
    ...
    for file in os.listdir(output_folder):
        if file.endswith('pretty.txt'):
            ...
            data2 = ' '.join(data)
            # print(data2,">>>")
            if ('Particulars' in data2  and 'Amount (Rs)' in data2):
                print('condition 1')
                a = type1(data)
                return a
...
def vivek_sharma_association(file_name, text, res_para, output_folder, db_conn,category,excel_id):
    # print(text,'\n\n')
    print("--------------","vivek sharma association format")
    
    time_stamp = datetime.now()

    invoice_number = re.search(r'(?si)invoice\sno.*?to',text).group()
    invoice_number = re.sub(r'(?si)date.*|Invoice\sNo\.\-|to','',invoice_number).strip()
    # print('invoice_number :',invoice_number,'\n\n')


    invoice_date = re.search(r'(?si)invoice\sno.*?to',text).group()
    ...
    if invoice_date in invoice_number:
        invoice_number = invoice_number.replace(f'{invoice_date}',"")
    ...
    if buyer_address == "Extract the buyer's address from the invoice text.":
        buyer_address = ''
    ...
    net_value = ''
    net_value = re.search(r'(?si)total.*?vivek\ssharma',text).group()
